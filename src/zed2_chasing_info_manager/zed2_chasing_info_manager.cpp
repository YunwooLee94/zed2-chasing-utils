...
void zed2_chasing_utils::ChasingInfoManager::DepthCallback(
  ...
  {
    rMin = std::max(int(zed_od.objects[idx].bounding_box_2d.corners[0].kp[1]) - param_.mask_padding_y, rGlobalMin);
    cMin = std::max(int(zed_od.objects[idx].bounding_box_2d.corners[0].kp[0]) - param_.mask_padding_x, cGlobalMin);
    rMax = std::min(int(zed_od.objects[idx].bounding_box_2d.corners[2].kp[1]) + param_.mask_padding_y, rGlobalMax);
    cMax = std::min(int(zed_od.objects[idx].bounding_box_2d.corners[2].kp[0]) + param_.mask_padding_x, cGlobalMax);
